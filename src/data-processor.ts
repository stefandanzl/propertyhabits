...
export class HabitDataProcessor {
    ...
    async extractHabitData(timeSpanKey: string): Promise<HabitData> {
        ...
        while (current.isSameOrBefore(end)) {
            const expectedPath = generateDailyNotePath(current, this.settings);
            ...
            if (fileHandler && fileHandler instanceof TFile) {
                ...
                try {
                    const metadata = this.app.metadataCache.getFileCache(fileHandler);
    ...
    getAvailableProperties(): Array<{ name: string; type: string }> {
        try {
            ...
            console.log("[Habit Tracker] Property entries:", Object.entries(allProperties));
