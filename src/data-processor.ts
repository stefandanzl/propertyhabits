...
import { handleError, processPropertyValue, getFilesInDateRange } from './utils';
...
export class HabitDataProcessor {
	...
	async extractHabitData(timeSpanKey: string): Promise<HabitData> {
		...
		const relevantFiles = getFilesInDateRange(startDate, endDate, this.settings, allFiles);
		...
		for (const filePath of relevantFiles) {
			...
			try {
				...
				for (const habit of this.settings.trackedHabits) {
					...
					const processedValue = processPropertyValue(habit.widget, rawValue);
	...
	private extractDateFromPath(filePath: string): string | null {
		try {
			...
			const relativePath = filePath.replace(`${this.settings.baseDirectory}/`, '').replace('.md', '');
			
			// Try to parse the date using the configured format
			const parsedDate = moment(relativePath, this.settings.dateFormatPattern, true);
	...
	getAvailableProperties(): Array<{name: string, type: string}> {
		try {
			...
			return Object.entries(allProperties)
				.filter(([_, info]) => info.type === 'checkbox' || info.type === 'number')
				.map(([name, info]) => ({
					name,
					type: info.type
				}));
	...
	isRelevantDailyNote(file: TFile): boolean {
		return file.path.startsWith(this.settings.baseDirectory) && file.path.endsWith('.md');
	}
}
